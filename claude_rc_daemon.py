...
project (any tmux session, any name) are adopted, never duplicated.
...
class Project:
    ...
    def spawn_mode(self) -> str:
        return "worktree" if self.git else "same-dir"
...
def running_servers() -> dict[Path, Server]:
    ...
    for entry in os.scandir("/proc"):
        ...
        out.setdefault(cwd, Server(pid, cwd, deleted))
    return out
...
def start(cfg: Config, project: Project, taken: set[str], dry_run: bool) -> None:
    sess = session_name(cfg, project, taken)
    cmd = ["claude", "remote-control", "--name", project.name, "--spawn", project.spawn_mode, *cfg.claude_args]
    if dry_run:
    ...
    taken.add(sess)
    # Mirror the pane to a log file so an early exit (auth, eligibility, trust prompt) stays diagnosable.
    try:
        cfg.log_dir.mkdir(parents=True, exist_ok=True)
        logfile = cfg.log_dir / f"{sess}.log"
        subprocess.run(["tmux", "pipe-pane", "-t", sess, "-o", f"cat >> {shlex.quote(str(logfile))}"], check=False)
    except OSError as exc:
        LOG.warning("cannot set up log for %s: %s", sess, exc)
    LOG.info("started %s for %s (%s)", sess, project.path, project.spawn_mode)


def stop(server: Server, cfg: Config, dry_run: bool) -> None:
    ...
    def reconcile(self) -> None:
        ...
        for path, project in desired.items():
            if path in running and not running[path].cwd_deleted:
                if path in self.started_at and now - self.started_at[path] > 120:
                    self.failures.pop(path, None)
                    self.next_allowed.pop(path, None)
                    self.started_at.pop(path, None)
                continue
            if path in self.started_at:
                n = self.failures.get(path, 0) + 1
                self.failures[path] = n
                delay = min(30 * 2 ** n, 3600)
                self.next_allowed[path] = now + delay
                self.started_at.pop(path, None)
                sess = self.cfg.session_prefix + project.name
                LOG.warning("server for %s exited (%d failures); retry in %ds. See %s, or run `claude` once "
                            "in that folder if it stalled on the workspace-trust prompt.",
                            path, n, delay, self.cfg.log_dir / f"{sess}.log")
                continue
            ...
            try:
                start(self.cfg, project, taken, self.dry_run)
                if not self.dry_run:
    ...
    def status(self) -> None:
        ...
        for path in sorted(set(desired) | {p for p in running if p.parent in self.cfg.hot_paths}):
            ...
            if path in desired and srv:
                ...
                state = "stray"
            print(f"{state:<10} {srv.pid if srv else '':>7}  {path}")
